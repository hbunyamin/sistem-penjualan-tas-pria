...
if (isset($_POST['SubmitTerimaBarang'])) {
    $IDTransaksi = $_POST['IDTransaksi'];
    $StatusPembayaran = 'Transaksi Selesai';

    $query = "UPDATE Transaksi SET StatusPembayaran = '$StatusPembayaran'
            WHERE IDTransaksi = $IDTransaksi";
    $hasil = mysqli_query($conn, $query);

    if ($hasil){
        echo "<script>alert('Konfirmasi Terima Barang Berhasil'); history.go(-2);</script>";
    } else{
        echo "<script>alert('Konfirmasi Terima Barang Gagal')</script>";
    }
}
...
<div id="spasi">
    ...
    <form method="POST" enctype="multipart/form-data">
        <table id="kotakBelanjaan">
            ...
            <tr>
                <td colspan="3">
                    Total Transaksi: <?php echo "Rp", number_format($a['SUM(k.TotalBelanjaan)'] + $a['OngkosKirim'] ,0,'','.'); ?>
                </td>
            </tr>
            <tr>
                <td colspan="3" align="center">
                    ...
                    if ($StatusPembayaran == 'Belum Bayar') {
                        echo "<B>TOTAL HARGA YANG HARUS DIBAYAR: Rp" . number_format(($TotalBelanjaan + $OngkosKirim),0,'','.') . "</B> <br>";
                    ...
                    ?>
                       <br><button name="SubmitTerimaBarang" class="lihatproduk" type="submit"
                           onclick="return confirm('Yakin Ingin konfirmasi terima barang ?')">
                            Konfirmasi terima barang </button>
